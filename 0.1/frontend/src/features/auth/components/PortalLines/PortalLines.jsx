...
export function PortalLines() {
  ...
  useEffect(() => {
    ...
    class Line {
      ...
      generate() {
        ...
        for (let index = 0; index < this.length; index += 1) {
          ...
          path += ` ${fragment.line}`
          magnitude += fragment.mag
        }

        this.line = { path, mag: magnitude }
        return this
      }
      ...
      renderStyle() {
        ...
        if (this.style === 'glitches') {
          ...
          context.setLineDash([this.size ** 1.45, (this.line.mag / this.length) * this.size ** 1.8])
          this.offset += 16
      ...
      mutatePath() {
        ...
        fragments[(Math.random() * fragments.length) | 0] = generators[(Math.random() * generators.length) | 0].line
    ...
    function clear() {
      context.fillStyle = 'hsla(228deg, 45%, 7%, 0.22)'
    ...
    function render() {
      ...
      if (frameId % 3 === 0) {
        ...
        lines.forEach((line) => {
          line.draw()
          if (frameId % 5 === 0 && Math.random() > 0.965) line.mutatePath()
        })
      }
  ...
  return <canvas ref={canvasRef} className={styles.linesCanvas} aria-hidden="true" />
}
